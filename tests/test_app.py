...
class TestLanding:
    ...
    def test_contains_puzzle_links(self, client):
        html = client.get("/").data.decode()
        assert "symbol-grid" in html
        assert "kenken" in html


class TestSymbolGridIndex:
    def test_returns_200(self, client):
        resp = client.get("/symbol-grid")
        assert resp.status_code == 200

    def test_contains_preset_buttons(self, client):
        html = client.get("/symbol-grid").data.decode()
        for name in PRESET_ORDER:
            assert f'value="{name}"' in html


class TestSymbolGridGenerate:
    def test_preset_easy(self, client):
        resp = client.post("/symbol-grid/generate", data={"preset": "Easy", "count": "1"})
        assert resp.status_code == 200
    ...
    def test_preset_without_answers(self, client):
        resp = client.post(
            "/symbol-grid/generate", data={"preset": "Easy", "count": "1"}
        )
        html = resp.data.decode()
        assert "Answer Key" not in html

    def test_multiple_puzzles(self, client):
        resp = client.post("/symbol-grid/generate", data={"preset": "Easy", "count": "4"})
        html = resp.data.decode()
        assert "Puzzle 1" in html
    ...
    def test_invalid_count(self, client):
        resp = client.post(
            "/symbol-grid/generate", data={"preset": "Easy", "count": "3"}
        )
        html = resp.data.decode()
    ...
    def test_all_presets_generate(self, client, preset):
        resp = client.post(
            "/symbol-grid/generate", data={"preset": preset, "count": "1"}
        )
        assert resp.status_code == 200
        assert b"Puzzle 1" in resp.data
